...
export class BrowsingHistoryController {
  ...
  async getHistory(@CurrentPortalUser() user: any, @Query() paginationDto: PaginationDto) {
    const result = await this.browsingHistoryService.getHistory(user.id, paginationDto);
    return {
      ...result,
  ...
  async trackVisit(@CurrentPortalUser() user: any, @Body() trackVisitDto: TrackVisitDto) {
    const entry = await this.browsingHistoryService.trackVisit(user.id, trackVisitDto);
    return {
      data: entry,
  ...
  async clearHistory(@CurrentPortalUser() user: any) {
    const result = await this.browsingHistoryService.clearHistory(user.id);
    return {
      data: result,
  ...
  async deleteEntry(@CurrentPortalUser() user: any, @Param('id') entryId: string) {
    await this.browsingHistoryService.deleteEntry(user.id, entryId);
    return {
  ...
  async getStats(@CurrentPortalUser() user: any) {
    const stats = await this.browsingHistoryService.getStats(user.id);
    return {
      data: stats,
