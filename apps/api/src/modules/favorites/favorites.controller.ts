...
export class FavoritesController {
  ...
  async getFavorites(@CurrentPortalUser() user: any, @Query() paginationDto: PaginationDto) {
    const result = await this.favoritesService.getFavorites(user.id, paginationDto);
    return {
      ...result,
  ...
  async addFavorite(@CurrentPortalUser() user: any, @Body() createFavoriteDto: CreateFavoriteDto) {
  ...
  async removeFavorite(@CurrentPortalUser() user: any, @Param('id') favoriteId: string) {
    await this.favoritesService.removeFavorite(user.id, favoriteId);
    return {
  ...
  async checkFavorite(@CurrentPortalUser() user: any, @Param('monumentId') monumentId: string) {
    const result = await this.favoritesService.checkFavorite(user.id, parseInt(monumentId));
    return {
      data: result,
