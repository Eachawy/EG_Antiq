...
- `docker/api.prod.Dockerfile` - Production environment with multi-stage build
...
### Unified NGINX Architecture (Production)

The production deployment uses a single unified NGINX instance as the main entry point for all Kemetra services.
...
└────────────────────────────────────────┘
      │           │           │
      ▼           ▼           ▼
  API Backend  Admin FE   Portal FE
  (NestJS)     (React)    (Next.js)
  Port 3000    Port 3001  Port 3002
```

**Services**:
- **API Backend** (EG_Antiq): NestJS application on port 3000
- **Admin Frontend** (EG_Antiq_backend): JHipster React on port 3001
- **Portal Frontend** (EG_Antiq_portal): Next.js standalone on port 3002

**Domain Routing**:
| Domain | Destination | Purpose |
|--------|-------------|---------|
| api.kemetra.org | localhost:3000 | API endpoints, Swagger docs |
| admin.kemetra.org | localhost:3001 | Admin dashboard |
| kemetra.org | localhost:3002 | Public portal |
| www.kemetra.org | → kemetra.org | SEO redirect |

**Key Features**:
- **Centralized CORS**: Single origin whitelist for all services
- **Rate Limiting**: API (100 req/min), Auth endpoints (10 req/min), Frontends (200 req/min)
- **SSL/TLS**: Single Let's Encrypt certificate for all domains
- **Security Headers**: HSTS, X-Frame-Options, CSP, X-Content-Type-Options
- **Enhanced Logging**: Request timing metrics for performance monitoring
- **Health Checks**: All services monitored via dedicated health endpoints

**Configuration File**: `nginx-configs/unified-kemetra.conf`

**Benefits**:
- Simplified SSL management (single certificate, unified renewal)
...
**Production:**
- Port: 5434 (host), 5433 (container)
- User: `postgres`
- Password: Set via `DATABASE_PASSWORD` environment variable
- Database: `antiq_production`
- Connection string from host: `postgresql://postgres:PASSWORD@localhost:5434/antiq_production`
- Connection string from container: `postgresql://postgres:PASSWORD@postgres:5433/antiq_production`

**Note:** The production database is only accessible from localhost (127.0.0.1) for security.
...
**Deployment Type**: Single-server monolithic deployment with unified NGINX gateway

**Server Details**:
- Server IP: 153.92.209.167
- All services containerized via Docker Compose
- Single NGINX instance handles all traffic

**Production Domains**:
- API Backend: https://api.kemetra.org
- Admin Frontend: https://admin.kemetra.org
- Portal Frontend: https://kemetra.org (+ www.kemetra.org)

**Port Mapping**:
| Service | Container Port | Host Binding | Public Access |
|---------|---------------|--------------|---------------|
| API Backend | 3000 | 127.0.0.1:3000 | Via NGINX only |
| Admin Frontend | 80 | 127.0.0.1:3001 | Via NGINX only |
| Portal Frontend | 3000 | 127.0.0.1:3002 | Via NGINX only |
| PostgreSQL | 5433 | 127.0.0.1:5434 | Localhost only |
| Redis | 6379 | 127.0.0.1:6379 | Localhost only |
| NGINX | 80, 443 | 0.0.0.0:80, 443 | Public |

**Note**: All backend services bind to localhost (127.0.0.1) for security. Only NGINX is publicly accessible.
...
```bash
cd /root
git clone https://github.com/your-org/EG_Antiq.git
git clone https://github.com/your-org/EG_Antiq_backend.git
git clone https://github.com/your-org/EG_Antiq_portal.git
```
...
# CRITICAL: Generate unique secrets for JWT_SECRET and PORTAL_JWT_SECRET
openssl rand -base64 64  # Use for JWT_SECRET
openssl rand -base64 64  # Use for PORTAL_JWT_SECRET (must be different!)
```
...
docker compose -f docker-compose.production.yml --profile ssl-setup run certbot \
  certonly --webroot --webroot-path=/var/www/certbot \
  --email admin@kemetra.org --agree-tos --no-eff-email \
  -d api.kemetra.org -d admin.kemetra.org -d kemetra.org -d www.kemetra.org
```
...
# Build and start all services (API, Admin, Portal, NGINX, PostgreSQL, Redis)
...
cd /root/EG_Antiq
docker compose -f docker-compose.production.yml up -d --build admin-frontend
```
...
# Rebuild portal frontend
cd /root/EG_Antiq
docker compose -f docker-compose.production.yml up -d --build portal-frontend
```
...
cd /root/EG_Antiq_backend && git pull origin main
cd /root/EG_Antiq_portal && git pull origin main
...
The unified NGINX configuration handles CORS for all services. Allowed origins are configured in:
- **NGINX**: `nginx-configs/unified-kemetra.conf` (CORS map section)
- **API**: `.env.production` (`CORS_ORIGINS` variable)

**CORS Origin Whitelist** (configured in both locations):
```
https://kemetra.org
https://www.kemetra.org
https://admin.kemetra.org
https://api.kemetra.org
```

**How CORS Works**:
1. NGINX handles CORS headers for static frontends
2. API handles CORS for its own endpoints
3. Both use the same whitelist for consistency

**Testing CORS**:
```bash
# Test API CORS from Admin frontend
curl -I -X OPTIONS https://api.kemetra.org/api/v1/auth/login \
  -H "Origin: https://admin.kemetra.org" \
  -H "Access-Control-Request-Method: POST"

# Test API CORS from Portal frontend
curl -I -X OPTIONS https://api.kemetra.org/api/v1/portal/auth/login \
  -H "Origin: https://kemetra.org" \
  -H "Access-Control-Request-Method: POST"

# Should return headers:
# Access-Control-Allow-Origin: https://admin.kemetra.org (or https://kemetra.org)
...
```bash
# Certbot renews automatically, but you can test renewal:
...
- Certificates: `./certs/live/kemetra.org/` (or first domain in certificate)
- Private key: `./certs/live/kemetra.org/privkey.pem`
- Full chain: `./certs/live/kemetra.org/fullchain.pem`
...
docker compose -f docker-compose.production.yml --profile ssl-setup run certbot \
  ...
  -d api.kemetra.org -d admin.kemetra.org -d kemetra.org -d www.kemetra.org -d new.kemetra.org

# Update nginx-configs/unified-kemetra.conf to handle new domain
# Reload NGINX
docker compose -f docker-compose.production.yml exec nginx nginx -s reload
```
...
**CORS Errors**:
- Check NGINX config is loaded: `docker compose -f docker-compose.production.yml exec nginx nginx -t`
- Verify origin is in whitelist (both NGINX and API .env)
- Check browser console for specific CORS error
- Test CORS with curl (see CORS Configuration section)
- Ensure NGINX is properly proxying requests to backend
...
# Test which backend NGINX is routing to
curl -I https://api.kemetra.org/api/v1/health
curl -I https://admin.kemetra.org/
curl -I https://kemetra.org/
...
curl http://localhost:3002/  # Portal
```
...
docker compose -f docker-compose.production.yml logs admin-frontend
docker compose -f docker-compose.production.yml logs portal-frontend

# Check service health
docker compose -f docker-compose.production.yml exec api wget -q -O- http://localhost:3000/api/v1/health
...
docker compose -f docker-compose.production.yml exec nginx \
  openssl x509 -in /etc/letsencrypt/live/kemetra.org/fullchain.pem -text -noout | grep "Not After"

# Renew certificate manually
docker compose -f docker-compose.production.yml run --rm certbot renew

# Check certificate is loaded in NGINX
openssl s_client -connect api.kemetra.org:443 -servername api.kemetra.org | grep "subject="
...
- Check NGINX rate limit configuration in `nginx-configs/unified-kemetra.conf`
- Current limits: API (100 req/min), Auth (10 req/min), Frontend (200 req/min)
- View NGINX error logs for rate limit messages
- Adjust limits in NGINX config if legitimate traffic
...
- Check API logs for email service errors:
  ```bash
  docker compose -f docker-compose.production.yml logs api | grep -i email
  ```
...
- Test API endpoint directly: `curl https://api.kemetra.org/api/v1/monuments/1`

**Database Connection Failed**:
- Check `DATABASE_URL` in `.env.production`
- Verify PostgreSQL container is running: `docker compose -f docker-compose.production.yml ps postgres`
- Check database is accessible from API container:
  ```bash
  docker compose -f docker-compose.production.yml exec api sh -c 'nc -zv postgres 5433'
  ```
...
```bash
# Check build context paths in docker-compose.production.yml
# Admin: context: ../EG_Antiq_backend
# Portal: context: ../EG_Antiq_portal

# Ensure repositories are cloned in correct locations
ls -la /root/EG_Antiq
ls -la /root/EG_Antiq_backend
ls -la /root/EG_Antiq_portal

# Manual build to see detailed error
cd /root/EG_Antiq
docker compose -f docker-compose.production.yml build admin-frontend --no-cache
```
...
docker compose -f docker-compose.production.yml exec nginx cat /var/log/nginx/access.log
